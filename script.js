...
window.addEventListener('load',()=>{
  setTimeout(()=>document.getElementById('loader').classList.add('out'),2800);
});
...
nbtn.addEventListener('click',()=>{
  ...
  nlbl.textContent=night?'Light Mode':'Night Mode';
  updateParticles();
});
...
hbg.addEventListener('click',()=>nav.classList.toggle('open'));
...
document.querySelectorAll('a,button,.sk').forEach(el=>{
  el.addEventListener('mouseenter',()=>{cur.style.width='14px';cur.style.height='14px';ring.style.width='48px';ring.style.height='48px';ring.style.opacity='.8'});
...
/* ── GLOBAL PARTICLE BG ──────────────────── */
const bgCanvas=document.getElementById('pc');
...
/* ── STACK CARD EFFECT: smoke in light mode, fireflies in night mode ── */
document.querySelectorAll('.sk').forEach(card=>{
  const cv=card.querySelector('.sk-canvas');
  ...
  /* touch: tap to trigger, auto-stop after 3s */
  card.addEventListener('touchstart',()=>{
    active=true;resize();pts=[];if(!raf)loop();
    clearTimeout(card._skTimer);
    card._skTimer=setTimeout(()=>{active=false;},3000);
  },{passive:true});
});
